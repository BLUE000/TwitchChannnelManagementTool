...
### 4.2. TransCipher による難読化設定の保存・復元
* **難読化キーの保護**:
  本体コアプログラム内に定義された固定の共通難読化キー（`QByteArray`）を利用します。このキーは `ICoreContext::getCipherKey()` 経由で、ロードされたプラグインにのみ安全に共有されます。
* **プラグイン側の保存処理（実装詳細）**:
  プラグインは設定データを保存する際、一時ファイルを用いず直接メモリ上でJSONからバイト配列へと変換し、`CipherEngine::encrypt` を呼び出したのち、専用フォルダの `config.bin` に直接書き出します。
* **安全対策 (DPAPIの併用)**:
